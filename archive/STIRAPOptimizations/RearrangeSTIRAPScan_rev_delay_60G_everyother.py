...
def build():
    ...
    REV_DELAY_US = [round(float(v), 4) for v in np.linspace(-1.0, 1.0, 11)]
    g().Pushout.STIRAPReverseDelay.scan(1, [float(v) * 1e-6 for v in REV_DELAY_US])
    ...
    # ---- run params (runp) ---------------------------------------------------------------
    rp.NumPerGroup = 143  # = rep x n_points (13 x 11, reverse-delay scan)
