...
def build():
    ...
    # ===== 2026-09-07 quad_10x10 COARSE freq-2D (this geometry's round 1 for the day) =====
    # quad_10x10 is the DENSE end of the geometry series (r_NN 10 um, z = 4, S_finite 4.12 --
    # the largest in the set), so its two-photon resonance sits well BELOW the dilute
    # geometries': in the 08-27/28 campaign quad_10x10 locked at (118.1500, 228.8300) while
    # double_spacing sat at (119.0523, 230.4586) and dimer_40um at (118.9419, 230.3707), i.e.
    # the dense array is ~1.6 MHz LOW in EOM616 and ~0.9 MHz low in carrier -- the
    # density-dependent interaction shift this whole series measures.
    # Today's quadruple_spacing (dilute, S = 0.00098) locked at (119.2466, 230.6808), which is
    # ~+0.22-0.31 MHz above the 08-27 dilute locks. Carrying that same offset onto the 08-27
    # quad_10x10 lock predicts today's dense optimum near (118.35, 229.05) -- an INDIRECT
    # estimate (a cross-geometry extrapolation, not a measurement), so the box below is kept
    # generous rather than tight around it.
    #   EOM616 228.00-231.00 -- brackets the 229.05 estimate with >= 1 MHz of margin each way
    #     and still reaches today's dilute lock 230.68 at the top, so if the interaction shift
    #     has collapsed the ridge is still inside the box.
    #   carrier 117.40-120.20 -- the ridge (slope ~0.84) crosses that EOM span diagonally.
    # FINE 7x7 about the coarse best cell (118.400, 229.000), survival 0.4462 +- 0.0182,
    # interior on both axes (data_20260907_192421). The coarse map REPRODUCES the 08-27
    # quad_10x10 lock almost exactly -- 0.4462 here vs 0.4475 +- 0.0104 then, at (118.40,
    # 229.00) vs (118.1500, 228.8300) -- so the dense-array working point has barely moved
    # even though the dilute geometries shifted ~+0.3 MHz today. The cross-geometry estimate
    # written above ((118.35, 229.05)) landed within one grid step of the measurement.
    # NOTE the floor here is SHALLOW (survival ~0.45 vs ~0.045 on quadruple_spacing): that is
    # the expected dense-array physics (r_NN 10 um -> vdW blockade suppresses excitation), not
    # a bad pulse, so contrast per cell is weak and the fine grid is read with that in mind.
    # Ridge from the four transferring rows: carrier = 118.8491 + 0.9551*(EOM616 - 229.5).
    #CARRIER_LIST_MHZ = [round(float(v), 4) for v in np.linspace(117.40, 120.20, 15)]  # coarse, 0.2 step
    #EOM616_LIST_MHZ = [round(float(v), 4) for v in np.linspace(228.00, 231.00, 7)]    # coarse, 0.5 step
    CARRIER_LIST_MHZ = [round(float(v), 4) for v in np.linspace(118.10, 118.70, 7)]    # dim 1, 0.1 step
    EOM616_LIST_MHZ = [round(float(v), 4) for v in np.linspace(228.625, 229.375, 7)]   # dim 2, 0.125 step
    ...
    g().Init.EOM616.Freq.scan(2, [float(v) * 1e6 for v in EOM616_LIST_MHZ])
    ...
    g().Pushout.STIRAPDelay = 1.333e-6
    ...
    g().rearrange_kwargs.extras.pattern = "quad_10x10"   # 2026-09-07 (this file's own geometry)
    ...
    # ---- run params (runp) ---------------------------------------------------------------
    rp.NumPerGroup = 245  # = rep x n_points (5 x 49, quad_10x10 fine) so the dashboard total matches the real cap
...
def RearrangeSTIRAPScan(url=None, reps=10):
    ...
    if reps is not None:
        opts["rep"] = reps
    did = ybStartScan(RearrangeSTIRAPSeq, g, url=url, label="RearrangeSTIRAPScan_fwd_freq2d_60G_quad10x10",
                      description=(
