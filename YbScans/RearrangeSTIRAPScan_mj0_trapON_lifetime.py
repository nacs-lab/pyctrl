...
# LOCKED VRydTrap=0.2 trap-ON forward-STIRAP optimum.
CH1_CARRIER_MHZ = 143.289
EOM616_MHZ = 281.967
VRYD_TRAP = 0.2
PW556_US = 7.0
PW308_US = 7.0   # matched ridge (pw556 = pw308)
DELAY_US = 0.8
...
def RearrangeSTIRAPScan(url=None, reps=6):
    ...
    if reps is not None:
        opts["rep"] = reps
    opts["description"] = (
        "mj=0 Rydberg LIFETIME (STIRAPGap sweep), TRAP-ON VRydTrap=0.2 (2026-07-22). "
        "STIRAPPushoutStep trap-off block COMMENTED OUT -> tweezers ON at VRydTrap=0.2 during the "
        "pulse. Locked at the 0.2 forward-STIRAP optimum: 556 143.289 / EOM616 281.967 MHz, "
        "pw556=pw308 7.0us / delay +0.8us (0.2 ridge interior), IfReverse 1 (reverse STIRAP "
        "de-excites Rydberg->ground after the gap; Ch2 carriers matched, reverse pw 7/7, "
        ...
        "survival." % ng)
    did = ybStartScan(RearrangeSTIRAPSeq, g, url=url, label="STIRAPmj0_trapON_lifetime", **opts)
    print("submitted RearrangeSTIRAPScan_mj0_trapON_lifetime -> descriptor id %s (url=%s, reps=%s, "
