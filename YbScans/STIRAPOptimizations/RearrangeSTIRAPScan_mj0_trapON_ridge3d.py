...
# LOCKED trap-OFF mj=0 two-photon pair (freq-2D data_20260722_143947; 89.6% exc). Trap OFF during
# pulse (STIRAPPushoutStep trap-off block ENABLED); VRydTrap=0.2 is pre-ramp only.
CH1_CARRIER_MHZ = 143.044
EOM616_MHZ = 281.444
VRYD_TRAP = 0.2

# Matched ridge pw556 = pw308 (axis 1) x STIRAPDelay (axis 2). Bracket the trap-off seeds
# (pw 6.0, delay +1.0) generously; pick interior best, extend if railed.
PW_US = [round(float(v), 4) for v in np.linspace(4.0, 8.0, 5)]        # pw556 = pw308, us -- axis 1
DELAY_US = [round(float(v), 4) for v in np.linspace(0.4, 1.6, 4)]     # STIRAPDelay, us -- axis 2
PW556_PTS = PW_US                                                     # pw308 mirrors pw556
...
def build():
    ...
    g().AWG.AWG308.Ch1.pulse_width_us.scan(1, PW556_PTS)      # pw308 MIRRORS pw556, axis 1
...
def RearrangeSTIRAPScan(url=None, reps=4):
    ...
    n2 = g().Pushout.STIRAPDelay.size(2)
    opts = {}
    if reps is not None:
        opts["rep"] = reps
    opts["description"] = (
        "mj=0 forward-STIRAP ridge-3D (matched pw x delay), MEASURED TRAP-OFF (2026-07-22; trap off "
        "during pulse, VRydTrap=0.2 pre-ramp only). Freqs LOCKED at the trap-off pair from the freq-2D "
        "(data_20260722_143947): carrier 143.044 / EOM616 281.444 (89.6%% exc at seed pulses). "
        "Re-optimize the pulse: "
        "pw556=pw308 %s us (axis1) x STIRAPDelay %s us (axis2), %dx%d grid, pattern "
        "quadruple_no_topright on 33x33_feedback11, IfReverse 0. Metric: verify-conditioned survival "
        "(LOW=best excitation); re-pair by Params (runbook Rule 2), check interior."
        % (PW_US, DELAY_US, n1, n2))
    did = ybStartScan(RearrangeSTIRAPSeq, g, url=url, label="STIRAPmj0_trapOFF_ridge3d", **opts)
    print("submitted RearrangeSTIRAPScan_mj0_trapON_ridge3d -> descriptor id %s (url=%s, reps=%s, "
          "verify=%s, NumImages=%d, grid=%dx%d)" % (did, url or "default", reps, VERIFY_IMAGE,
                                                    3 if VERIFY_IMAGE else 2, n1, n2))
    return did
