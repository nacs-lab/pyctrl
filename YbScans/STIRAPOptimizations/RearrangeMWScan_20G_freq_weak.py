...
def build():
    ...
    # 2026-08-12 ROUND 3 -- WEAK DRIVE. Rounds 1 (100 ns) and 2 (50 ns) anti-correlate at
    # 11330-11335 (peak <-> dip), confirming a resonance there, but at gain 8000 the drive is
    # strong enough (Omega ~10 MHz) that sidelobes compete and no clean single line appears.
    # Drop gain 8000 -> 2000 (Omega ~2.5 MHz) with a 200 ns ~pi pulse: one narrow dip, no sidelobes.
    MW_FREQ_MHZ = [round(float(v), 6) for v in np.linspace(11320.0, 11346.0, 27)]   # 1 MHz steps
    g().QICK.freq.scan(1, MW_FREQ_MHZ)                # SWEPT dim 1
    g().QICK.gain = 2000                              # ROUND 3 weak drive: Omega ~2.5 MHz
    g().QICK.duration = 0.2e-6                        # ~pi at Omega ~2.5 MHz
