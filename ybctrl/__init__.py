"""ybctrl -- launch-shim package for the pyctrl run loop.

The pyctrl modules are imported FLAT (``from exp_seq import ExpSeq``) off the pyproject
``pythonpath`` (lib / YbSteps / YbSeqs / YbExptCtrl / tools). This ``ybctrl`` package exists
only to provide the ``python -m ybctrl.run_loop.runner <url>`` entry point the monitor's
