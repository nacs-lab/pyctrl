...
def RearrangeSTIRAPScan(url=None, reps=4):
    ...
    if reps is not None:
        opts["rep"] = reps
    opts["description"] = (
        "mj=-1 forward-STIRAP freq-2D, TRAP-ON VRydTrap=0.2 (2026-07-22 STEP 1). Tweezers ON at "
        "VRydTrap=0.2 during the pulse (STIRAPPushoutStep trap-off block COMMENTED OUT). Relocate the "
        "mj=-1 two-photon pair after the 308-polarization fix: carrier556 142.5-144.5 @10 x EOM616 "
        "233.3-235.3 @10, centered on the 30 G line 143.5 (AT-confirmed) and the post-fix mj-1 308 "
        "line ~234.27 (AT data_20260722_155653). Delay +1.0us, pw556 6.0/pw308 5.7 seeds, pattern "
        "quadruple_no_topright, IfReverse 0. Metric: verify-conditioned survival (LOW=best excitation).")
    did = ybStartScan(RearrangeSTIRAPSeq, g, url=url, label="STIRAPmjm1_trapON_freq2D", **opts)
