...
def build():
    ...
    REV_AMP556 = [round(float(v), 4) for v in np.linspace(0.5, 1.0, 5)]
    REV_AMP308 = [round(float(v), 4) for v in np.linspace(0.6, 1.0, 5)]
    g().AWG.AWG556.Ch2.amplitude_scale.scan(1, REV_AMP556)
    g().AWG.AWG308.Ch2.amplitude_scale.scan(2, REV_AMP308)
    ...
    g().Init.EOM616.Freq = 230.6131e6                # every_other forward RE-LOCK (09-07)
    ...
    # ---- run params (runp) ---------------------------------------------------------------
    rp.NumPerGroup = 250  # = rep x n_points (10 x 25, reverse-amplitude 2-D)
