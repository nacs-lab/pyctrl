...
def build_sine(params):
    """Sine: the SIMPLEST program -- one const (DDS) sine tone. ``duration`` (s), ``freq`` (MHz),
    ``phase`` (deg), ``gain`` (DAC). No pi/2 or wait structure.

    channels ch0 = ``[Sine]`` (or ``[loop(N,[Sine])]`` when ``duration`` exceeds the single-pulse HW
    length cap -- :func:`_split_duration` chunks it). ``duration`` falls back to ``drive_time`` if
    unset (so a scan that only set the Rabi knob still works). Sweep any of freq/gain/phase/duration.
    """
    freq = params["freq"]
    gain = params["gain"]
    phase = params.get("phase", 0.0)
    dur_s = params.get("duration", params.get("drive_time"))
    if dur_s is None:
        raise ValueError("Sine template needs 'duration' (s) -- set g().QICK.duration")
    n_loops, chunk_ns = _split_duration(float(dur_s) * 1e9)
    pulses = {"Sine": simple_pulse_cfg("Sine", freq, gain, chunk_ns, phase=phase)}
    body = "Sine" if n_loops == 1 else loop(n_loops, ["Sine"])
    channels = [[body]]
    # KEY carries every consumed param so a sweep of any (freq/gain/phase/duration) mints a distinct
    # program (a key missing a swept field collapses all points to one upload -> silent no-op).
    key = ("Sine", float(freq), float(gain), float(phase), float(dur_s))
