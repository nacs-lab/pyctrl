...
# ---- the fine box (4 x 4 x 4 = 64 combos), centred on the round-1 ridge corner ----
PW556_US = [6.2, 6.5, 6.8, 7.0]        # scan dim 1  (7.0 = hardware ceiling, round-1 rail)
PW308_US = [5.2, 5.7, 6.2, 6.7]        # scan dim 2  (<= 7 us ceiling)
DELAY_US = [0.6, 0.9, 1.2, 1.6]        # scan dim 3  (308 first; MUST stay < PadTime 2 us)
...
def RearrangeSTIRAPScan(url=None, reps=4):
    ...
    if reps is not None:
        opts["rep"] = reps
    opts["description"] = (
        "2026-08-01 forward-STIRAP FINE 3D round 2 (runbook step 2b): pw556 %s x pw308 %s x "
        "delay %s us = %d combos, reverse OFF, freqs locked at carrier %.3f / EOM616 %.3f. Centred "
        "on the round-1 ridge corner (coarse data_20260801_115310: best 7.0/6.7/0.6 = 96.44%%, pw556 "
        "railed at the 7us ceiling, delay marginal still climbing -> extended to 1.6). Repeated cell "
        "(7.0, 6.7, 0.6) = this round's drift anchor. Pattern double_spacing on 33x33_feedback11, "
        "VRydTrap 2, 3-image (verify). Metric: TARGET-ONLY verify-conditioned; group by Params."
        % (PW556_US, PW308_US, DELAY_US,
           len(PW556_US) * len(PW308_US) * len(DELAY_US), CARRIER_MHZ, EOM616_MHZ))
    did = ybStartScan(RearrangeSTIRAPSeq, g, url=url, label="STIRAPfwd_fine3d", **opts)
    print("submitted RearrangeSTIRAPScan_fwd_fine3d -> descriptor id %s (url=%s, reps=%s, "
