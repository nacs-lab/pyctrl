...
def build():
    ...
    g().AWG.AWG556.Ch1.carrier_freq_MHz.scan(1, list(np.linspace(142.9, 144.1, 10)))   # 2026-07-21 mj=0 quad freq-2D ZOOM: carrier broad, keep 142.9-144.1 @10
    g().AWG.AWG556.Ch1.pulse_width_us = 5.5   # mj=0 plateau-center pw556; ridge-3D re-optimizes
    ...
    g().AWG.AWG308.Ch1.pulse_width_us = 5.2   # mj=0 plateau-center pw308; ridge-3D re-optimizes
    ...
    g().Init.EOM616.Freq.scan(2, list(np.linspace(281.4e6, 282.6e6, 10)))   # 2026-07-21 mj=0 quad freq-2D ZOOM: EOM616 282.0 +/-0.6 @10 (0.133 step); coarse (data_20260721_175748) dip 143.5/282.0=5.8%
    g().Pushout.VRydTrap = 2
    g().Pushout.BiasCoilCurrent.Ryd = 30
    g().Pushout.STIRAPDelay = 1.0e-6   # 2026-07-21 mj=0 delay-prescan plateau center (data_20260721_174943; window +0.6..+1.4us, best +1.2us=4.0%)
    ...
    # ---- run params (runp) ---------------------------------------------------------------
    rp.NumPerGroup = 100 * 4   # 2026-07-21 mj=0 quad freq-2D ZOOM: 100 pts (10x10) x 4 reps (pass rep=4)
...
def RearrangeSTIRAPScan(url=None, reps=10):
    ...
    if reps is not None:
        opts["rep"] = reps
    opts["description"] = (
        "mj=0 QUADRUPLE_SPACING forward-STIRAP freq-2D ZOOM 2026-07-21 (STEP 1b). carrier556 "
        "142.9-144.1 @10 x EOM616 281.4-282.6 @10 (0.133 step) at delay +1.0us. Zoom of "
        "the coarse 9x7 (data_20260721_175748, aborted @218/252) whose interior dip was 143.5/282.0 = "
        "5.8% surv (94% excitation) on a clean degenerate diagonal line; EOM616 is the sensitive axis "
        "so tightened to +/-0.6. pw556 5.5 / pw308 5.2, VRydTrap 2 (trap-off block on -> pre-ramp only), "
        "IfReverse 0. Deepest-dip cell = mj=0 two-photon pair. Metric: verify-conditioned survival (LOW=best).")
    did = ybStartScan(RearrangeSTIRAPSeq, g, url=url, label="STIRAPmj0quad_freq2D_zoom", **opts)
