"""RearrangeSTIRAPScan_fwd_freq2d_20G.py -- 2026-08-12 forward-STIRAP FREQUENCY 2D at 20 G.

20 G copy of ``RearrangeSTIRAPScan_fwd_freq2d.py`` (which is 30 G). Only the bias field, the
swept frequency box and the parked Ch2 carrier differ -- the pulse (PW556/PW308/DELAY) is
carried over unchanged from the 30 G verify winner, per the runbook's "seed the new field from
the converged pulse, re-pin the resonance first" order. See the 20 G VARIANT block below.

--- original 30 G header follows ---
...
# ---- 20 G VARIANT (2026-08-12) -----------------------------------------------------
# Retuned from the 30 G box (carrier 142.9-144.1 / EOM616 233.5-234.7 around the 07-31 lock
# 143.5/234.1) to the 20 G two-photon pair measured today:
#   * carrier seed 131.8467 MHz = the 20 G single-photon 556 line (scan 20260812102327,
#     FWHM 174.6 kHz, R^2 0.977; an independent pass at a stronger push agreed to 3.5 kHz),
#   * EOM616 seed 236.0263 MHz = the 20 G 616 revival peak with the 556 parked on that line
#     (scan 20260812105321, R^2 0.986).
# The carrier seed is sharp (kHz); the EOM616 seed comes from a 14.8 MHz-wide power-broadened
# revival, so it is only good to a few hundred kHz -- hence the EOM616 span is 2.5x WIDER than
# the carrier span. Note the implied pair offset at 20 G is +104.2 MHz vs +90.6 at 30 G, so do
# NOT carry the 30 G offset over; the box is centred on the measured pair, not on the offset.
CARRIER_LIST_MHZ = [round(float(v), 4) for v in np.linspace(131.25, 132.45, 10)]  # scan dim 1
...
def build():
    ...
    g().AWG.AWG556.Ch2.carrier_freq_MHz = 131.85  # reverse OFF; parked at the 20 G line
    ...
    g().Pushout.BiasCoilCurrent.Ryd = 20   # 2026-08-12: 20 G variant (was 30)
