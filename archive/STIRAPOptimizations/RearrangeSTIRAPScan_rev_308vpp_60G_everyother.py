...
def build():
    ...
    # ===== 2026-09-07 ROUND 8d: DELAY SCAN (campaign step 3) at the R8c forward lock =====
    # Frequencies now FIXED at the along-ridge floor found by the co-vary
    # (data_20260907_185348): argmin EOM616 230.6690 / carrier 119.2373 at survival
    # 0.0445 +- 0.0060, parabolic-refined to (230.6808, 119.2466) -- used here. The floor is
    # BROAD (6 points within 1 SEM, EOM616 ~230.25-231.08 all at 0.044-0.055) and rises hard
    # only above 231.2, so this pair is a floor representative, not a resolved argmin; the
    # fixed-point verify decides. The delay is the one forward knob never measured on THIS
    # geometry -- 1.333 us was adopted from the every_other 60 G lock -- and the runbook calls
    # it the knob that sets whether transfer happens at all, so it is worth its own scan.
    CARRIER_LIST_MHZ = None
    EOM616_LIST_MHZ = None
    ...
    # ===== 2026-09-07 every_other: VERIFY THE EXISTING 08-27 LOCKS, do not re-optimize =====
    # User directive: check the recorded locks first and only optimize if they no longer hold.
    # Forward lock (08-27, along-ridge co-vary argmin data_20260827_164918): carrier 118.3375 /
    # EOM616 229.6655 / delay 1.35 us -> survival 0.0836 +- 0.0046 (excitation 91.64%).
    # Reverse lock (data_20260827_170604): Ch2 556 carrier 118.5428, reverse delay 0.0,
    # widths 2.0/2.0, amp 0.9 -> round-trip return ~0.92 (08-19 floor).
    # 2026-09-07 RE-LOCK verify. Forward pair from today's along-ridge co-vary
    # (data_20260907_204710): argmin (230.6131, 119.1430), survival 0.0936 +- 0.0061, a clean
    # interior minimum -- the two-photon resonance has moved ~+0.95 MHz in EOM616 since 08-27.
    g().AWG.AWG556.Ch1.carrier_freq_MHz = 119.1430   # every_other forward RE-LOCK (09-07)
    ...
    # Reverse carrier TRACKS the forward one at the geometry's measured offset: the 08-27 pair
    # was reverse 118.5428 - forward 118.3375 = +0.2053 MHz (the same "offset" convention the
    # chain-baseline records for the adopted geometries). Carried onto today's forward re-lock:
    # 119.1430 + 0.2053 = 119.3483. If the round trip does NOT recover with this, the reverse
    # carrier gets its own 1-D scan rather than another assumed offset.
    # ===== 2026-09-07 every_other REVERSE frequency scan (campaign step 4) =====
    # The forward re-lock recovered only part of the loss and the ROUND TRIP did not recover at
    # all: at the re-locked forward pair (119.1430, 230.6131) the 200-shot verify
    # (data_20260907_210356) read forward excitation 88.91% +- 0.21% (up from 86.94% at the old
    # lock, still under the 91.64% record) but return 0.5580 +- 0.0051 vs the ~0.92 record. So
    # the reverse leg is genuinely off, not merely mistracked -- carrying the 08-27 forward-to-
    # reverse offset (+0.2053 MHz) onto the new forward carrier was NOT enough. This gives the
    # reverse its own sweep instead of another assumed offset, per the campaign protocol
    # (rev freq: lock -0.5 / +1.5, 29 pts x 13 reps), centred on the new FORWARD lock.
    # IfReverse is pinned to 1 here: we are maximizing the round-trip RETURN (higher = better),
    # the opposite sense to the forward rounds.
    # Reverse carrier FIXED at the measured argmax of the reverse-frequency sweep
    # (data_20260907_211247): 119.3573, return 0.6643 +- 0.0299. That sweep showed the carrier is
    # NOT what is wrong -- its peak sat within one grid step of the offset-tracked guess
    # (119.3483) and the whole 2 MHz curve is broad and shallow (0.46-0.66), nothing like a
    # mistuned-frequency signature. So this round sweeps the GAP instead.
    g().AWG.AWG556.Ch2.carrier_freq_MHz = 119.3573
    ...
    # ===== 2026-09-07 every_other REVERSE AMPLITUDE 2-D (556 Ch2 x 308 Ch2) =====
    # The reverse return is still only ~0.53-0.74 against the ~0.92 record and the three knobs
    # already scanned do not explain it: carrier peaks where predicted and is broad/shallow
    # (data_20260907_211247), the gap sweep extrapolates to 0.773 at 0.1 us with tau ~29 us so
    # nothing is decaying away (data_20260907_212903), and the reverse delay is worth ~1 sigma
    # (data_20260907_213740). Amplitudes set the adiabaticity of the de-excitation sweep and
    # have NOT been touched today, so they are scanned here as a 2-D grid.
    # NOTE 308 Ch2 is already at amplitude_scale 1 (its ceiling), so that axis can only explore
    # DOWNWARD; 556 Ch2 sits at 0.9 and can go to 1.0.
    # ===== 2026-09-07 every_other: 308 POWER x EOM616 for the ROUND TRIP =====
    # Tonight's revival pair CONFIRMED the 08-07 "over-power" hypothesis that had sat untested:
    # halving the 308 AOM amp 0.4 -> 0.2 narrowed the 60 G revival from FWHM 13.36 MHz
    # (data_20260907_221516) to 5.995 MHz (data_20260907_222009) -- back to the 5.47 MHz clean
    # reference -- collapsed the tilted baseline, and cost almost nothing in peak height
    # (0.91 -> 0.89). It also LIGHT-SHIFTED the centre +746 kHz (229.1871 -> 229.9332).
    # A broadened Rydberg line is exactly what kills a coherent round trip while barely touching
    # the forward (population transfer read out as loss), which is the asymmetry we measured.
    # So: does turning the STIRAP's OWN 308 down recover the return? Both AWG308 channels are
    # co-varied on dim 1 (the forward Ch1 pulse broadens the line during excitation AND the
    # reverse Ch2 pulse during de-excitation, so scaling only one would be a half test -- note
    # the earlier reverse-amp 2-D swept Ch2 alone and found nothing).
    # EOM616 rides dim 2 because power and resonance are COUPLED by that light shift: a
    # fixed-frequency power scan would walk off resonance and read as a fake power dependence.
    # 556 Ch2 pinned at the amp-2D best cell (data_20260907_215811). It MUST be set explicitly:
    # this file replaced that scan's amplitude block, and leaving it unset silently falls back
    # to the config default instead of the measured value.
    g().AWG.AWG556.Ch2.amplitude_scale = 0.875
    # ===== 2026-09-07: is the reverse capped by the AWG 308 DRIVE LEVEL? =====
    # The 308-amplitude sweep (data_20260907_223824) measured the honest de-excitation
    # efficiency eta = (R - S_fwd)/(1 - S_fwd) at each power and found it rising MONOTONICALLY
    # with 308 and PINNED AT THE CEILING: 0.423 / 0.501 / 0.521 / 0.525 / 0.559 for amp
    # 0.3/0.5/0.7/0.85/1.0. So the reverse is 308-power-LIMITED, not over-powered -- the
    # opposite of what the revival's over-power broadening suggested (that is the DDS/AOM drive
    # in a CW-like regime; this is the pulsed AWG drive, and the two are not the same regime).
    # amplitude_scale is already at 1, so the only remaining headroom is the drive level itself.
    # max_amplitude_vpp is honored since 2026-07-16; the 556 channels already run at 15 Vpp, so
    # 8 -> 14 is well inside what the instrument and the rest of this config already use.
    # If eta is flat across this sweep the 07-15 "saturated by 7.5" note still holds and the
    # limit is optical (308 power / alignment), which is hardware, not a scan knob.
    g().AWG.AWG308.Ch1.amplitude_scale = 1
    g().AWG.AWG308.Ch2.amplitude_scale = 1
    VPP308 = [8.0, 10.0, 12.0, 14.0]
    g().AWG.AWG308.Ch1.max_amplitude_vpp.scan(1, VPP308)
    g().AWG.AWG308.Ch2.max_amplitude_vpp.scan(1, VPP308)   # SAME dim = co-varied
    ...
    # (AWG308 Ch1 amplitude_scale is swept on dim 1 above, co-varied with Ch2.)
    ...
    # (308 Ch2 amplitude_scale is swept on dim 2 above -- see the reverse-amplitude block.)
    ...
    # EOM616 on dim 2, spanning the re-lock 230.6131 and ~+1.4 MHz above it: the revival showed
    # the line moves UP as 308 power comes down (+746 kHz for a halving), so the low-power
    # columns need frequency headroom on the high side or they read as failures.
    # EOM616 pinned on the forward re-lock. The previous round swept it with the carrier FIXED,
    # which simply walks OFF the two-photon resonance -- its high "returns" at 231.4/231.8
    # (0.86-0.98) are unexcited atoms trivially surviving, not good round trips.
    g().Init.EOM616.Freq = 230.6131e6
    ...
    # SIGNED seconds; the sweep axis for this round. 1.333 us (the adopted every_other value)
    # sits mid-span so the incumbent is measured head-to-head against its neighbours.
    # ===== 2026-09-07 ROUND 8e: FIXED-POINT VERIFY (runbook step 4) =====
    # Every forward knob pinned at the located optimum, 100 reps -> the campaign number with a
    # tight per-shot SEM. This is also the safety net the runbook demands against a bad
    # co-vary attribution (Rule 2). Delay CONFIRMED at the incumbent by R8d
    # (data_20260907_191041): argmin 1.300 us, parabolic-refined 1.333 us -- i.e. the value
    # adopted from every_other was already right for this geometry; floor 1.3-1.5 us at
    # survival 0.047-0.049, rising steeply below 1.1 (0.386 at 0.6 us).
    g().Pushout.STIRAPDelay = 1.35e-6   # every_other forward lock (08-27)
    ...
    # ===== 2026-09-07 every_other REVERSE DELAY scan (campaign step 5) =====
    # Last untested reverse knob. The others are now excluded: the reverse CARRIER peaks within
    # one grid step of the offset-tracked value and is broad/shallow (data_20260907_211247), and
    # the LIFETIME is not the culprit either -- the return-vs-gap sweep (data_20260907_212903)
    # extrapolates to only 0.773 +- 0.012 at gap 0.1 us and fits tau ~29 us, i.e. LONGER than
    # the 6.35 us every_other anomaly, so population is not decaying away before the reverse
    # pulse. That leaves the reverse pulse's own timing. Protocol: -1..+1 us, ~10x10; run here
    # at 11 pts x 13 reps with the gap held at 0.1 us.
    # Reverse delay FIXED at its measured argmax (data_20260907_213740, parabolic-refined).
    g().Pushout.STIRAPReverseDelay = 0.153e-6
    ...
    # ===== 2026-09-07 every_other: RETURN vs GAP -- is the bad round trip the PULSE or the
    # Rydberg LIFETIME? =====
    # every_other is the campaign's flagged anomaly: tau_S = 6.35 +- 0.16 us (08-28 re-measure)
    # against 50-145 us for every other geometry. The forward+reverse verifies above all ran at
    # a FIXED 1 us hold, which at that lifetime already costs exp(-1/6.35) = 0.854 of the excited
    # population before the reverse pulse even fires -- capping the achievable return near 0.87,
    # not 0.92. Sweeping the gap separates the two hypotheses cleanly:
    #   * gap -> 0 recovers ~0.85-0.9  => the reverse PULSE is fine and the loss is lifetime
    #     (and today's tau_S is shorter than 6.35 us), nothing pulse-tuning can fix;
    #   * gap -> 0 stays ~0.66         => the reverse pulse itself is inefficient, keep tuning it.
    # The fitted intercept is also the honest round-trip efficiency, and the decay constant is
    # today's tau_S -- directly comparable to the 08-28 anomaly number.
    # Gap pinned SHORT (0.1 us, the shortest point of the return-vs-gap sweep) so this round
    # measures reverse-PULSE efficiency with the lifetime term as small as possible.
    g().Pushout.STIRAPGap = 0.1e-6

    # Sweep IfReverse over [0, 1] in ONE scan (the 08-12 efficiency-verify trick) so forward and
    # reverse are measured back-to-back under identical conditions -- drift cannot separate them:
    #   IfReverse=0 -> no de-excitation, survival LOW  = 1 - forward excitation  (expect ~0.084)
    #   IfReverse=1 -> full round trip,  survival HIGH = round-trip return       (expect ~0.92)
    # IfReverse on dim 2 so BOTH legs are measured at every 308 power. Return alone is not
    # interpretable for a round trip -- survival -> 1 both when the round trip is perfect and
    # when nothing was excited. With both legs the honest figure of merit is the de-excitation
    # efficiency  eta = (R - S_fwd) / (1 - S_fwd),  which divides out how much was excited.
    g().Pushout.IfReverse.scan(2, [0, 1])
    g().Pushout.IfPump = 0
    ...
    # ---- run params (runp) ---------------------------------------------------------------
    rp.NumPerGroup = 200  # = rep x n_points (25 x 8, 308-vpp x IfReverse)
    ...
    # 1 here: this round sweeps STIRAPDelay only and holds EOM616 FIXED, so the "Scramble 0 iff
    # EOM616 swept" rule does not apply and scrambling is preferred (it decorrelates slow drift
    # from the swept axis).
    rp.Scramble = 1   # EOM616 is FIXED this round, so scrambling is preferred
    rp.isGrid2 = 0
    rp.isInit = 0
    rp.isHC = 0
