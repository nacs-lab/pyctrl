...
def build():
    ...
    # 2026-09-16 TOP-N VERIFY of the freq-2D (data_20260916_180001, 4 shots/cell). The grid's
    # top 4 cells span 0.3190..0.3413 with SEMs 0.0054..0.0180 -- i.e. ~1 sigma apart, NOT
    # resolved -- and the incumbent lock ranked 5/117 at 0.3630 +- 0.0106, only 2.1 sigma off
    # the argmin. Locking a 4-shot/cell argmin is exactly the error stage B made and stage C
    # corrected today, so the 4 best cells + the INCUMBENT are re-run at 100 shots each.
    # CO-VARY PATH: carrier and EOM616 BOTH on scan dim 1, so point k is the PAIR
    # (CARRIER_LIST_MHZ[k], EOM616_LIST_MHZ[k]) -- pairing by construction. Runbook Rule 2:
    # the lab analysis would collapse and re-order this, so the read-out MUST group by Params.
    #   k=0  96.9036 / 392.6017   grid rank 1  (0.3190 +- 0.0180)
    #   k=1  97.0036 / 392.7517   rank 2       (0.3310 +- 0.0116)
    #   k=2  96.8036 / 392.6017   rank 3       (0.3406 +- 0.0110)
    #   k=3  96.6036 / 392.3017   rank 4       (0.3413 +- 0.0054)
    #   k=4  96.7036 / 392.4517   rank 5 = INCUMBENT (0.3630 +- 0.0106)
    # All five carried 1930-2080 mid events in the grid (within 0.93-1.00x the median), so no
    # candidate is here on a small-N fluke.
    CARRIER_LIST_MHZ = [96.9036, 97.0036, 96.8036, 96.6036, 96.7036]   # dim 1 (co-vary)
    EOM616_LIST_MHZ  = [392.6017, 392.7517, 392.6017, 392.3017, 392.4517]  # dim 1 (co-vary)
    assert len(CARRIER_LIST_MHZ) == len(EOM616_LIST_MHZ)
    ...
    g().AWG.AWG308.Ch1.pulse_width_us = 3.0
    ...
    g().Init.EOM616.Freq.scan(1, [float(v) * 1e6 for v in EOM616_LIST_MHZ])  # dim 1: co-vary
    ...
    rp.NumPerGroup = REPS * len(CARRIER_LIST_MHZ)  # co-vary path: 5 pairs x 100 = 500
    ...
    rp.Scramble = 0  # *** dim 1 sweeps Init.EOM616.Freq: a SCRAMBLED EOM sweep kicks the
                     # 616 out of lock (gotcha-scramble-unlocks-616-eom-sweep). MUST be 0. ***
    rp.isGrid2 = 0
    rp.isInit = 0
    rp.isHC = 0
...
def RearrangeSTIRAPScan_fwd_freqverify_60G_mjp1(url=None, reps=REPS):
    ...
    if reps is not None:
        opts["rep"] = reps
    did = ybStartScan(RearrangeSTIRAPSeq, g, url=url, label="RearrangeSTIRAPScan_fwd_freqverify_60G_mjp1",
                      description=(
                          "STIRAP FREQ TOP-N VERIFY 09-16 at 100 shots/pt. Operating point "
                          "3P1 mj=+1 / 60 G / 66 3S1, every_other on 33x33_feedback11. Resolves "
                          "the flat ridge bottom of today's freq-2D (data_20260916_180001, "
                          "4 shots/cell), whose top 4 cells span 0.3190-0.3413 with SEMs "
                          "0.0054-0.0180 (~1 sigma apart, NOT resolved) and whose incumbent lock "
                          "ranked 5/117 at 0.3630 +- 0.0106, only 2.1 sigma off the argmin. "
                          "CO-VARY PATH on dim 1 -- 5 (carrier, EOM616) pairs: "
                          "(96.9036,392.6017) rank1 / (97.0036,392.7517) rank2 / "
                          "(96.8036,392.6017) rank3 / (96.6036,392.3017) rank4 / "
                          "(96.7036,392.4517) rank5 = INCUMBENT, 100 reps each = 500 shots. All "
                          "five carried 1930-2080 mid events in the grid, so none is a small-N "
                          "fluke. PULSE AREA PINNED to the stage-C verified incumbent "
                          "(pw556 3.0 / pw308 3.0 / delay 1.35 us) so this measures FREQUENCY "
                          "ONLY. Freq-2D ridge: carrier = 97.269 + 1.02*(EOM616 - 392.948), "
                          "R^2 0.975 -- NOTE the slope moved 1.25 -> 1.02 vs 09-15, outside the "
                          "quoted +-0.06, an OPEN question for yb-physics. Analysis: MINIMIZE "
                          "survival; group by the logged Params (runbook Rule 2 -- run_analysis "
                          "would collapse and re-order this co-vary path) and use the "
                          "target-masked verify-conditioned metric (Rule 1). IfReverse=0. "
                          "*** Scramble=0 because dim 1 sweeps EOM616. ***"),
                      **opts)
    print("submitted freqverify_60G_mjp1 -> descriptor id %s (url=%s, reps=%s, verify=%s, "
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    RearrangeSTIRAPScan_fwd_freqverify_60G_mjp1(url=args.url, reps=args.reps)
