...
def build():
    ...
    # 2026-08-12 fine reverse-delay scan: jobs 913/914 were flat within +-0.3 us and their
    # argmax hopped; a pooled parabola put the peak at +0.06 +- 0.02 us vs the standing -0.089.
    # 8 pts x 25 reps, drift-free within ONE scan, to settle it.
    REV_DELAY_US = [round(float(v), 4) for v in np.linspace(-0.15, 0.20, 8)]
    g().Pushout.STIRAPReverseDelay.scan(1, [v * 1e-6 for v in REV_DELAY_US])
