...
def build():
    ...
    CARRIER_LIST_MHZ = [round(float(v), 4) for v in np.linspace(118.90, 119.50, 7)]   # dim 1, 0.1 step
    EOM616_LIST_MHZ = [round(float(v), 4) for v in np.linspace(230.125, 230.875, 7)]  # dim 2, 0.125 step
    ...
    # ---- run params (runp) ---------------------------------------------------------------
    rp.NumPerGroup = 245  # = rep x n_points (5 x 49, R8b fine) so the dashboard total matches the real cap
...
def RearrangeSTIRAPScan(url=None, reps=10):
    ...
    if reps is not None:
        opts["rep"] = reps
    did = ybStartScan(RearrangeSTIRAPSeq, g, url=url, label="RearrangeSTIRAPScan_fwd_freq2d_60G_eo",
                      description=(
