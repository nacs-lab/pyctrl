...
from devices.qick_awg.templates import (MAX_LEN_NS, MIN_LEN_NS, NS_PER_CC,
                                        ...
                                        build_rabi, build_ramsey, build_sine)
...
# --------------------------------------------------------------------------- #
# Sine (single const tone: duration + freq + phase + gain)
# --------------------------------------------------------------------------- #
def test_sine_single_const_tone():
    prog = build_sine(_params("Sine", duration=1e-6, phase=45.0))
    assert prog.key == ("Sine", 10863.04, 3000.0, 45.0, 1e-6)
    assert set(prog.pulses) == {"Sine"}
    s = prog.pulses["Sine"]
    assert s["style"] == "const" and s["freq"] == 10863.04 and s["gain"] == 3000
    assert s["phase"] == 45.0 and s["length"] == pytest.approx(1000.0)   # 1 us -> ns
    assert prog.channels == [["Sine"]]                                    # single pulse, no loop


def test_sine_falls_back_to_drive_time():
    p = {"template": "Sine", "freq": 10863.04, "gain": 3000, "phase": 0.0, "drive_time": 300e-9}
    prog = build_sine(p)
    assert prog.key == ("Sine", 10863.04, 3000.0, 0.0, 300e-9)
    assert prog.pulses["Sine"]["length"] == pytest.approx(300.0)


def test_sine_chunks_long_duration():
    prog = build_sine(_params("Sine", duration=(3 * MAX_LEN_NS) * 1e-9))
    (body,) = prog.channels[0]
    assert isinstance(body, Loop) and body.body == ["Sine"] and body.count >= 3
