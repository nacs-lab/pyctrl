...
def build():
    ...
    P308 = [round(float(v), 4) for v in np.linspace(0.3, 1.0, 5)]
    ...
    EOM616_MHZ = [round(float(v), 4) for v in np.linspace(230.2, 231.8, 5)]
    g().Init.EOM616.Freq.scan(2, [float(v) * 1e6 for v in EOM616_MHZ])
    ...
    g().Pushout.IfReverse = 1   # round trip: the swept axis is the REVERSE carrier, maximize return
    g().Pushout.IfPump = 0
    ...
    # ---- run params (runp) ---------------------------------------------------------------
    rp.NumPerGroup = 250  # = rep x n_points (10 x 25, 308-power x EOM616)
    ...
    # from the swept axis).
    rp.Scramble = 0   # dim 2 sweeps Init.EOM616.Freq -- scramble + 616-EOM jumps risk a 616 unlock
    rp.isGrid2 = 0
    rp.isInit = 0
    rp.isHC = 0
