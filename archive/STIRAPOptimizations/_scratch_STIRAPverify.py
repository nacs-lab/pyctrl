...
# ---- FIXED-POINT VERIFY (single best candidate; 100 shots) --------------------------- #
# Ridge-3D global-min cell (data_20260721_182536), TARGET-ONLY + MID metric: 2.53% surv (97.47% exc).
PW556 = [6.0]                                           # us
PW308 = [5.7]                                           # us
DELAY = [1.0e-6]                                         # s
# ------------------------------------------------------------------------------------ #
...
def build():
    ...
    g().AWG.AWG556.Ch1.pulse_width_us.scan(1, PW556)   # 5-candidate co-vary path (dim1); from ridge-3D top-5
    ...
    g().AWG.AWG308.Ch1.pulse_width_us.scan(1, PW308)   # 5-candidate co-vary path (dim1); from ridge-3D top-5
    ...
    g().Init.EOM616.Freq = 282.067e6   # 2026-07-21 mj=0 quad freq-2D lock (data_20260721_181013, 96.5%): PAIR w/ carrier 143.567
    g().Pushout.VRydTrap = 2
    g().Pushout.BiasCoilCurrent.Ryd = 30
    g().Pushout.STIRAPDelay.scan(1, DELAY)   # 5-candidate co-vary path (dim1); from ridge-3D top-5
    ...
    # ---- run params (runp) ---------------------------------------------------------------
    rp.NumPerGroup = 1 * 100   # 2026-07-21 fixed-point verify: 1 candidate x 100 reps (pass rep=100)
...
def RearrangeSTIRAPScan(url=None, reps=10):
    ...
    if reps is not None:
        opts["rep"] = reps
    _delay_us = [round(d*1e6, 3) for d in DELAY]
    opts["description"] = (
        "mj=0 QUADRUPLE_SPACING forward-STIRAP FIXED-POINT VERIFY 2026-07-21 (STEP 3), 100 shots. "
        "Ridge-3D global-min cell (data_20260721_182536, TARGET-ONLY + MID metric = 97.47% exc): "
        "pw556 6.0 / pw308 5.7 / delay +1.0us, at the locked mj=0 resonance carrier 143.567 / EOM616 "
        "282.067 (freq-2D data_20260721_181013). VRydTrap 2 (trap-off block on -> pre-ramp only), "
        "IfReverse 0. Reports target-only P(final|mid) w/ per-shot SEM = campaign optimum.")
    did = ybStartScan(RearrangeSTIRAPSeq, g, url=url, label="STIRAPmj0quad_top5verify", **opts)
