...
def build():
    ...
    DELAY_LIST_US = [round(float(v), 4) for v in np.linspace(0.60, 2.00, 15)]
    g().Pushout.STIRAPDelay.scan(1, [float(v) * 1e-6 for v in DELAY_LIST_US])
    ...
    # ---- run params (runp) ---------------------------------------------------------------
    rp.NumPerGroup = 195  # = rep x n_points (13 x 15, R8d delay) so the dashboard total matches the real cap
