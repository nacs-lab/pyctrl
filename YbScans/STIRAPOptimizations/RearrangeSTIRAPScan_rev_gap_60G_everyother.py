...
def build():
    ...
    g().Pushout.STIRAPReverseDelay = 0.0  # every_other REVERSE lock (08-27)
    ...
    GAP_PTS_S = [float(v) * 1e-6 for v in np.geomspace(0.1, 20.0, 12)]
    g().Pushout.STIRAPGap.scan(1, GAP_PTS_S)
    ...
    # ---- run params (runp) ---------------------------------------------------------------
    rp.NumPerGroup = 180  # = rep x n_points (15 x 12, return-vs-gap scan)
