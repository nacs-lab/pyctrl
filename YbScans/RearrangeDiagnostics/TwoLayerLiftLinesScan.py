"""TwoLayerLiftLinesScan.py -- two-layer array whose FAR layer is a set of 1-D CHAINS.

Same protocol, optics and timing as TwoLayerLiftTriangleScan; only the far layer's target
geometry differs.  The far sublattice is exactly 16x16 (odd rows x odd cols of the every-other
checkerboard), so it tiles into sixteen 4x4 blocks with nothing left over.  Each block collapses
to ONE 16-atom line sitting directly over its own 5x5 near block -- so the whole array becomes
16 parallel 1-D chains, each hovering 10 um above the 5x5 patch it came from.

THE LINES RUN ON THE BLOCK DIAGONAL, and that is a resolution decision, not an aesthetic one.
Along a lattice axis a line has only the 80 um block pitch to live in, so 16 atoms force a
<=4.5 um spacing; a build at 4.0 um came back with the two-layer grid derive matching only
816-846 of 1068 sites (residual 27 knm px) and the target mask collapsing to ~315 ones instead
of 536 -- the far-plane peaks merge.  On the diagonal the along-line room is 80*sqrt(2) = 113 um
and neighbouring parallel lines sit 80/sqrt(2) = 57 um apart, which lets the spacing be set by
what the far-plane derive resolves rather than by what the tiling allows.  Camera detection was
never the limit either way (boxSize is 9 cam px = 1.6 um).

COST OF THE LONGER MOVE: the far atoms now travel up to 50 um to reach their line, against a
5 um stagger before.  Survival is set by the per-frame STROKE, not the step count, so nsteps is
raised to hold the stroke at round 0's proven 0.49 px/frame instead of shortening the move.

Original header follows.

build a TWO-LAYER array out of one loaded plane and read BOTH
...
THE PROTOCOL (4 camera frames, 3 SLM rounds; seq ``TwoLayerLiftCommSeq``)
  ...
  img4    the far layer, in focus, as 16 one-dimensional chains.  Detected on its own registry grid.
...
FULL_PHASE = "phase/fb11_2layer_lines_z10um.pt"   # 10 um: the working distance (0.96 vs 0.67 at 20)
FAR_PATTERN = "fb11_2layer_lines_far"
FAR_PHASE = "phase/fb11_2layer_lines_far.pt"
TARGET_BITS_JSON = os.path.join(ROOT, "_twolayer_lift", "target_bits_lines_z10um.json")
...
LAT_STAGGER_PX = 95.29                 # MAX assignment move for the 1-D lines layout, knm px
                                       # (38.89 um: 4 um spacing along the row, offset half a
                                       #  base pitch so the line clears the stationary layer)
...
LIFT_NSTEPS = 195              # round 1 -- 0.49 px/frame lateral, 0.064 rad/step axial.
                               # Scaled from the triangular scan's 40 steps because the move
                               # grew from a 12.2 px stagger to a 123.5 px assignment.  The
                               # per-frame STROKE is what costs survival, not the step count,
                               # so it is held at round 0's proven 0.49 px/frame (24.5 px in
                               # 50 steps, measured AT the detection floor) rather than at
                               # the triangular scan's more conservative 0.31.
...
def main():
    ...
    ap.add_argument("--label", default="TwoLayerLiftLinesRearrangeScan",
