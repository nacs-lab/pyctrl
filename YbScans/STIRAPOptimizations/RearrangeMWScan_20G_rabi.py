...
def build():
    ...
    # ---- QICK microwave (out-of-band; run loop uploads one program per swept freq) -------
    # 2026-08-12 MW RABI at 20 G. f0 = 11334.93 +- 0.02 MHz from the low-power limit of the
    # revival-destruction gain ladder (gains 8000/4000/2000/1000/500 -> FWHM 15.7/3.8/1.07/0.34/0.21
    # MHz, FWHM ~ gain^1.59). The gain-8000 centre (11334.33) is saturation-shifted 0.6 MHz LOW --
    # do not use it. Gains 1000 and 500 agree to 0.7 sigma.
    # Drive at gain 2000: Omega ~2.5 MHz (period ~400 ns) so 5 samples/period covers 4 us in 50 pts.
    # (Omega ~10 MHz at gain 8000, inferred from the 2pi->pi inversion between the 100 ns and 50 ns
    # STIRAP scans -- NOT the 4.825 MHz in the legacy scan file.)
    g().QICK.template = "Sine"
    g().QICK.freq = 11334.93                          # FIXED at the low-power f0
    g().QICK.gain = 2000
    # ROUND 2: round 1 (0.02-4 us, 81 ns steps) showed ALL the dynamics inside the first ~200 ns --
    # survival 0.276 at a 20 ns pulse, then a flat ~0.55 incoherent plateau out to 4 us. So Omega is
    # much faster than the 2.5 MHz assumed for gain 2000 (a 20 ns pulse is already near pi), and the
    # 81 ns sampling only ever saw the plateau. Zoom to 0-0.5 us at ~12 ns steps.
    MW_TIME_S = [round(float(v), 12) for v in np.linspace(0.01e-6, 0.5e-6, 40)]
    g().QICK.duration.scan(1, MW_TIME_S)              # SWEPT dim 1: Rabi pulse length
    g().QICK.rabi_freq = 2.5e6
    ...
    # gap == duration exactly, matching the established RearrangeMWScan convention. An earlier
    # version used duration + 0.5 us, which only added MW-free Rydberg hold at every point.
    g().Pushout.STIRAPGap.scan(1, MW_TIME_S)
    ...
    g().rearrange_kwargs.extras.pattern = "eightfold_spacing"   # 8x spacing; verified 2026-08-12 job 941: fwd 95.6%, round trip 91.7% #"quadruple_no_topright"  # 2026-07-10 quadruple_spacing (post optics move) -- the new default for the 33x33 array
