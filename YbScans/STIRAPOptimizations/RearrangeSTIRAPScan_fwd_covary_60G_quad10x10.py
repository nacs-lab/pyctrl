...
def build():
    ...
    # quad_10x10 ridge from its own fine 7x7 (data_20260907_194547):
    #   carrier = 118.2882 + 0.7862*(EOM616 - 229.0)
    # anchored on that scan's best cell (118.200, 228.750), survival 0.4154 +- 0.0116, interior
    # on both axes. Span chosen to walk +-0.8 MHz of EOM616 about the fine optimum, which also
    # reaches the coarse best (229.00) and the 08-27 lock (228.83) so all three are compared
    # inside one drift-free scan.
    N_COVARY = 30
    EOM616_LIST_MHZ = [round(float(v), 4) for v in np.linspace(228.00, 229.60, N_COVARY)]
    CARRIER_LIST_MHZ = [round(118.2882 + 0.7862 * (e - 229.0), 4) for e in EOM616_LIST_MHZ]
