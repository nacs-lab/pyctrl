...
def build():
    ...
    # 2026-09-16 PULSE-AREA REOPEN: pw308 (dim 1) x amp556 amplitude_scale (dim 2).
    # RATIONALE (user, 09-16): the 308 leg is POWER-LIMITED -- its amplitude_scale is already at
    # the 1.0 ceiling -- so the only way to raise its adiabatic pulse AREA is to lengthen it.
    # The 556 leg carries ~4x the power, so its amplitude is the genuinely free knob and is
    # scanned rather than pinned.
    # WHY THIS MATTERS: the mj=+1 campaign has never searched past pw 4.0 us. Stage B swept
    # pw556 x pw308 over 3.0-4.0 only, stage C verified 3.0/3.0, and the frequency work today
    # held them there -- yet the 08-01 mj=0 verified lock ran pw556 6.8 / pw308 6.2 us and
    # reached 95.4% excitation (data_20260801_121937), vs ~65% now. The search has plausibly
    # been stuck well BELOW the pulse area that works, so pw308 is opened to the ceiling.
    # *** pulse_width_us must NOT exceed 7 us (hardware ceiling, per user 2026-07-31). ***
    PW308_LIST_US = [3.0, 4.0, 5.0, 6.0, 7.0]           # dim 1, up to the hardware ceiling
    AMP556_LIST   = [0.4, 0.55, 0.7, 0.85, 1.0]         # dim 2, brackets the current 0.87
    # FREQUENCIES PINNED at the INCUMBENT, which today's top-N verify kept: the best alternative
    # beat it by only 1.8 sigma and the grid ranking reshuffled under 9x the statistics
    # (data_20260916_182720, 187/500 shots -- the run was cut short by a DAQmx -200018 NI DAC
    # underflow, a known intermittent runtime bug, NOT a physics result).
    CARRIER_MHZ = 96.7036
    EOM616_MHZ  = 392.4517
    ...
    g().AWG.AWG556.Ch1.amplitude_scale.scan(2, [float(v) for v in AMP556_LIST])  # SWEPT dim 2
    ...
    g().Pushout.STIRAPDelay = 1.35e-6  # PINNED (stage-C verified incumbent)
    ...
    rp.NumPerGroup = REPS * len(PW308_LIST_US) * len(AMP556_LIST)  # 12 x 5 x 5 = 300
...
def RearrangeSTIRAPScan_fwd_pw308amp556_60G_mjp1(url=None, reps=REPS):
    ...
    if reps is not None:
        opts["rep"] = reps
    did = ybStartScan(RearrangeSTIRAPSeq, g, url=url, label="RearrangeSTIRAPScan_fwd_pw308amp556_60G_mjp1",
                      description=(
                          "STIRAP PULSE-AREA REOPEN 09-16: AWG308.Ch1.pulse_width_us (dim 1, "
                          "3/4/5/6/7 us, to the 7 us hardware ceiling) x "
                          "AWG556.Ch1.amplitude_scale (dim 2, 0.4/0.55/0.7/0.85/1.0) = 25 combos "
                          "x 12 passes = 300 shots. Operating point 3P1 mj=+1 / 60 G / 66 3S1, "
                          "every_other on 33x33_feedback11. RATIONALE (user): the 308 leg is "
                          "POWER-LIMITED with amplitude_scale already at its 1.0 ceiling, so its "
                          "adiabatic pulse AREA can only be raised by lengthening it; the 556 leg "
                          "carries ~4x the power so its amplitude is the free knob and is scanned. "
                          "WHY IT MATTERS: the mj=+1 campaign has NEVER searched past pw 4.0 us "
                          "(stage B swept 3.0-4.0, stage C verified 3.0/3.0) and sits at ~65% "
                          "excitation, while the 08-01 mj=0 verified lock ran pw556 6.8 / pw308 "
                          "6.2 us for 95.4% (data_20260801_121937) -- the search looks stuck well "
                          "below the working pulse area. pw556 held at 3.0 and delay at 1.35 us so "
                          "this isolates the 308 area; BOTH will likely need re-optimizing after, "
                          "since delay sets the 556<->308 overlap and the widths are coupled. "
                          "Frequencies PINNED at the incumbent (carrier 96.7036 / EOM616 392.4517) "
                          "which today's top-N verify kept (best alternative only 1.8 sigma, "
                          "ranking reshuffled). Analysis: MINIMIZE survival; group by the logged "
                          "Params (Rule 2) and use the target-masked verify-conditioned metric "
                          "(Rule 1); 2-D decode is COLUMN-MAJOR, dim-0 (pw308) fastest. "
                          "IfReverse=0. Scramble=1 (no EOM sweep; decorrelates the +0.115 "
                          "within-run loading drift seen today)."),
                      **opts)
    print("submitted pw308amp556_60G_mjp1 -> descriptor id %s (url=%s, reps=%s, verify=%s, "
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    RearrangeSTIRAPScan_fwd_pw308amp556_60G_mjp1(url=args.url, reps=args.reps)
