...
def build():
    ...
    # 2026-09-16 PULSE WIDTH x DELAY (the correct pairing; supersedes the aborted pw308 x
    # amp556 grid, job 118 / data_20260916_184540).
    # WHY THE PREVIOUS DESIGN WAS WRONG (user, 09-16): the delay is NOT an independent knob --
    # it is the PARTNER of the pulse width. It sets the 556<->308 overlap, and with compact-
    # support quintic splines (playback total = pw) the overlap geometry is fixed jointly by
    # (pw, delay). Crossing pw308 against amp556 while HOLDING delay at 1.35 us therefore left
    # every long-pulse cell at the wrong overlap, so that axis measured mismatch, not area.
    # Job 118's pw308 marginal (0.497/0.554/0.710/0.743/0.768 for 3..7 us at 2 shots/cell) is
    # uninterpretable for exactly this reason and is NOT evidence against long 308 pulses.
    # Pulse width must always be co-scanned with the delay -- that is what this scan does.
    PW308_LIST_US = [3.0, 4.0, 5.0, 6.0, 7.0]     # dim 1, to the 7 us hardware ceiling
    DELAY_LIST_US = [0.5, 1.5, 2.5, 3.5, 4.5]     # dim 2, the overlap partner
    # *** DELAY MUST BE STRICTLY POSITIVE. STIRAPPushoutStep fires the 308 gate, waits
    # Forward_Delay, then fires the 556 gate, so delay <= 0 is not representable and raises at
    # sequence-BUILD time, per shot, killing the whole job (learned 09-15, job 2073). ***
    # pw556 stays 3.0 to isolate the 308 leg (the user's power-limited-308 hypothesis); the
    # delay is now FREE to re-establish the right overlap at each 308 length, which is the
    # test job 118 failed to perform.
    # amp556 PROVISIONALLY 1.0: job 118's pw308=3.0 row was cleanly monotonic in amp556
    # (0.788/0.583/0.441/0.367/0.303 for 0.40..1.00), so the ceiling was best -- but that is
    # 2 shots/cell and gets its own 1-D scan next, per the user.
    CARRIER_MHZ = 96.7036
    EOM616_MHZ  = 392.4517
    ...
    g().AWG.AWG556.Ch1.pulse_width_us = 3.0
    g().AWG.AWG556.Ch1.max_amplitude_vpp = 14
    g().AWG.AWG556.Ch1.amplitude_scale = 1.0  # provisional ceiling (1-D amp scan to follow)
    ...
    g().AWG.AWG308.Ch1.pulse_width_us.scan(1, [float(v) for v in PW308_LIST_US])  # SWEPT dim 1
    ...
    g().Pushout.STIRAPDelay.scan(2, [float(v) * 1e-6 for v in DELAY_LIST_US])  # SWEPT dim 2
    ...
    rp.NumPerGroup = REPS * len(PW308_LIST_US) * len(DELAY_LIST_US)  # 12 x 5 x 5 = 300
...
def RearrangeSTIRAPScan_fwd_pw308delay_60G_mjp1(url=None, reps=REPS):
    ...
    if reps is not None:
        opts["rep"] = reps
    did = ybStartScan(RearrangeSTIRAPSeq, g, url=url, label="RearrangeSTIRAPScan_fwd_pw308delay_60G_mjp1",
                      description=(
                          "STIRAP PULSE WIDTH x DELAY 09-16: AWG308.Ch1.pulse_width_us (dim 1, "
                          "3/4/5/6/7 us to the 7 us hardware ceiling) x Pushout.STIRAPDelay "
                          "(dim 2, 0.5/1.5/2.5/3.5/4.5 us, all > 0) = 25 combos x 12 passes = "
                          "300 shots. Operating point 3P1 mj=+1 / 60 G / 66 3S1, every_other on "
                          "33x33_feedback11. SUPERSEDES the aborted pw308 x amp556 grid (job 118, "
                          "data_20260916_184540, aborted at 105 shots): per the user, the delay is "
                          "NOT independent -- it is the PARTNER of the pulse width, setting the "
                          "556<->308 overlap, so crossing pw against amp at a FIXED 1.35 us delay "
                          "left every long-pulse cell at the wrong overlap and measured mismatch "
                          "rather than pulse area. Job 118's monotonic pw308 marginal is "
                          "uninterpretable for that reason and is NOT evidence against long 308 "
                          "pulses. Here the delay is free to re-establish the overlap at each 308 "
                          "length -- the test job 118 failed to perform. pw556 held at 3.0 to "
                          "isolate the power-limited 308 leg; amp556 provisionally 1.0 from job "
                          "118's clean monotonic amp row at pw308=3.0 (2 shots/cell), with a "
                          "dedicated 1-D amp scan to follow. Frequencies pinned at the incumbent "
                          "(96.7036 / 392.4517). Analysis: MINIMIZE survival; group by the logged "
                          "Params (Rule 2), target-masked verify-conditioned metric (Rule 1); "
                          "2-D decode COLUMN-MAJOR, dim-0 (pw308) fastest. IfReverse=0. "
                          "Scramble=1 (no EOM sweep)."),
                      **opts)
    print("submitted pw308delay_60G_mjp1 -> descriptor id %s (url=%s, reps=%s, verify=%s, "
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    RearrangeSTIRAPScan_fwd_pw308delay_60G_mjp1(url=args.url, reps=args.reps)
