...
def build():
    ...
    CARRIER_LIST_MHZ = [round(float(v), 4) for v in np.linspace(96.1036, 97.3036, 13)]  # dim 1, 0.10
    EOM616_LIST_MHZ  = [round(float(v), 4) for v in np.linspace(391.8517, 393.0517, 9)]  # dim 2, 0.15
    ...
    rp.NumPerGroup = REPS * len(CARRIER_LIST_MHZ) * len(EOM616_LIST_MHZ)  # 4 x 13 x 9 = 468
    ...
    rp.Scramble = 0  # *** dim 2 sweeps Init.EOM616.Freq: a SCRAMBLED EOM sweep kicks the
...
def RearrangeSTIRAPScan_fwd_freq2d_60G_mjp1(url=None, reps=REPS):
    ...
    if reps is not None:
        opts["rep"] = reps
    did = ybStartScan(RearrangeSTIRAPSeq, g, url=url, label="RearrangeSTIRAPScan_fwd_freq2d_60G_mjp1",
                      description=(
                          "STIRAP FREQ-2D 09-16: two-photon resonance re-measure at 3P1 mj=+1 / "
                          "60 G / 66 3S1, every_other on 33x33_feedback11. PURPOSE: today's "
                          "stage-C top-N verify (20260916173225) reproduced the 09-15 RANKING but "
                          "read 2-4 pp WORSE survival on all three triples (2.1-3.9 sigma), which "
                          "is a level shift common to the run, not a pulse-area effect. The "
                          "frequency pair was inherited from 09-15 (data_20260915_162512) and "
                          "never re-measured today, and the 616 revival is known to walk "
                          "(-142 kHz in one evening on 09-14). This scan asks whether the "
                          "two-photon resonance has moved. GRID: carrier (dim 1) 96.1036-97.3036 "
                          "x 0.10 = 13 pts, EOM616 (dim 2) 391.8517-393.0517 x 0.15 = 9 pts, "
                          "117 combos x 4 passes = 468 shots. Window is DELIBERATELY IDENTICAL "
                          "to the 09-15 round 2 (+-0.60 x +-0.60), which was declared converged, "
                          "so a walk shows as a SHIFT OF THE ARGMIN in a known-good box; centre "
                          "is the current lock. PULSE AREA PINNED to the stage-C verified "
                          "incumbent (pw556 3.0 / pw308 3.0 / delay 1.35 us) so this measures "
                          "FREQUENCY ONLY. Ridge for re-derivation: carrier = 97.392 + "
                          "1.25*(EOM616 - 392.948); the two extreme EOM columns truncate -- "
                          "ignore their fits. Analysis: MINIMIZE survival; group by the logged "
                          "Params (runbook Rule 2) and use the target-masked verify-conditioned "
                          "metric (Rule 1). IfReverse=0. *** Scramble=0 because dim 2 sweeps "
                          "EOM616. ***"),
                      **opts)
    print("submitted freq2d_60G_mjp1 -> descriptor id %s (url=%s, reps=%s, verify=%s, "
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    RearrangeSTIRAPScan_fwd_freq2d_60G_mjp1(url=args.url, reps=args.reps)
