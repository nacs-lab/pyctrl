...
def RearrangeSTIRAPScan(url=None, reps=4):
    ...
    if reps is not None:
        opts["rep"] = reps
    opts["description"] = (
        "mj=-1 forward-STIRAP ridge-3D (matched pw x delay), TRAP-ON VRydTrap=0.2 (2026-07-22). "
        "Freqs LOCKED at the mj=-1 pair from the freq-2D (data_20260722_161150): carrier 142.944 / "
        "EOM616 233.967 (97.5%% exc at seed pulses). Re-optimize the pulse: "
    ...
    did = ybStartScan(RearrangeSTIRAPSeq, g, url=url, label="STIRAPmjm1_trapON_ridge3d", **opts)
    print("submitted RearrangeSTIRAPScan_mjm1_trapON_ridge3d -> descriptor id %s (url=%s, reps=%s, "
